...
def flowplot(u, psi, flow, binning, iangle):
    ...
    plt.subplot(3, 4, 12)
    plt.imshow(u[:, :, n//2], cmap='gray')
    if not os.path.exists('/local/data/vnikitin/lam/flowr_'+str(ntheta)+'_'+str(iangle)+'/'):
        os.makedirs('/local/data/vnikitin/lam/flowr_'+str(ntheta)+'_'+str(iangle)+'/')
    plt.savefig('/local/data/vnikitin/lam/flowr_'+str(ntheta)+'_'+str(iangle)+'/flow'+str(k))
    plt.close()
...
if __name__ == "__main__":
    ...
    phis = np.arange(-3,3,0.02)
    for iangle in range(len(phis)):
        phi = (61.18+phis[iangle])/180*np.pi
        det = data.shape[2]
        ntheta = data.shape[0]
        ...
        with lcg.SolverLam(det, det, det, det, ntheta, phi, 1e-2) as tslv:
            with dc.SolverDeform(ntheta, det, det, 42) as dslv:
                ...
                for k in range(niter):
                    ...
                    #         u,  '/local/data/vnikitin/lam/rec_align'+str(idset)+'_'+str(iangle)+'/tmp'+'_'+str(ntheta)+'_'+str(alpha)+'/rect'+str(k)+'/r', overwrite=True)
                    #     dxchange.write_tiff_stack(
                    #         psi1, '/local/data/vnikitin/lam/prj_align'+str(idset)+'_'+str(iangle)+'/tmp'+'_'+str(ntheta)+'_'+str(alpha)+'/psir'+str(k)+'/r',  overwrite=True)
                        # if not os.path.exists('flow'+str(alpha)+'_'+str(iangle)):
                        #     os.makedirs('/local/data/vnikitin/lam/flow'+str(alpha)+'_'+str(iangle))
                        # np.save('/local/data/vnikitin/lam/flow'+str(alpha)+'_'+str(iangle)+'/'+str(k),flow)
