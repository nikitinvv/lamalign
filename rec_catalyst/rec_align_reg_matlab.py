...
def flowplot(u, psi, flow, binning):
    ...
    plt.subplot(3, 4, 12)
    plt.imshow(u[:, :, n//2], cmap='gray')
    if not os.path.exists('flow_'+str(ntheta)+'/'):
        os.makedirs('flow_'+str(ntheta)+'/')
    plt.savefig('flow_'+str(ntheta)+'/flow'+str(k))
    plt.close()
        ...
        with dc.SolverDeform(ntheta, det, det) as dslv:
            ...
            for k in range(niter):
                ...
                flow = dslv.registration_flow_batch(
                    psi1, data, 0, 1, flow.copy(), pars, nproc=42)
                t1 = toc()
                tic()
                ...
                if(np.mod(k, 16) == 0):  # check Lagrangian
                    ...
                    dxchange.write_tiff_stack(
                        u,  'rec_align/tmp'+'_'+str(ntheta)+'_'+str(alpha)+'/rect'+str(k)+'/r', overwrite=True)
                    dxchange.write_tiff_stack(
                        psi1, 'prj_align/tmp'+'_'+str(ntheta)+'_'+str(alpha)+'/psir'+str(k)+'/r',  overwrite=True)
