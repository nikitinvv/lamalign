...
def flowplot(u, psi, flow, binning):
    ...
    plt.subplot(3, 4, 12)
    plt.imshow(u[:, :, n//2].real, cmap='gray')
    if not os.path.exists('/data/staff/tomograms/viknik/lamino/matlab_rec/flow/'+str(binning)+'_'+str(ntheta)+'/'):
        os.makedirs('/data/staff/tomograms/viknik/lamino/matlab_rec/flow/' +
                    str(binning)+'_'+str(ntheta)+'/')
    plt.savefig('/data/staff/tomograms/viknik/lamino/matlab_rec/flow/' +
...
if __name__ == "__main__":
    ...
    data = dxchange.read_tiff(
        '/data/staff/tomograms/viknik/lamino/matlab_rec/matlab-recon.tif').astype('complex64')[:, 8:-8, 8:-8]
    theta = np.load(
        '/data/staff/tomograms/viknik/lamino/matlab_rec/angle.npy').astype('float32')/180*np.pi
    print(theta)
    print(data.shape)
    print(theta.shape)
    # exit()
        ...
        with dc.SolverDeform(ntheta, det, det) as dslv:
            ...
            for k in range(niter):
                ...
                psi = dslv.cg_deform(data, psi, flow, 4,
                                     tslv.fwd_lam(cp.array(u), cp.array(theta)).get()+lamd/rho, rho, nproc=42)
                t2 = toc()
                # tomo subproblem
                tic()
                ...
                if(np.mod(k, 4) == 0):  # check Lagrangian
                    ...
                    dxchange.write_tiff_stack(
                        u.real,  '/data/staff/tomograms/viknik/lamino/matlab_rec/rec_align/tmp'+str(0)+'_'+str(ntheta)+'/rect'+str(k)+'/r', overwrite=True)
                    dxchange.write_tiff_stack(
                        psi.real, '/data/staff/tomograms/viknik/lamino/matlab_rec/prj_align/tmp'+str(0)+'_'+str(ntheta)+'/psir'+str(k)+'/r',  overwrite=True)
